...
class Saturation(Benchmark):
    ...
    def create_dataset(self):
        ...
        for idx, node in enumerate(white_nodes):
            ...
            if abs(red_count - blue_count) > 1 and min(red_count, blue_count) > 1:#差距大于1且少的颜色的个数大于1，列入测试中
            ...
            if blue_count > red_count:
                ...
                labels[node] = 2
        labels = [x - 1 for x in labels] #red:1 blue:0
        data = dgl.from_networkx(g)
        data.ndata['x'] = torch.nn.functional.one_hot(torch.tensor(colors)).float()#原本颜色的onehot编码 torch.nn.functional.one_hot
    ...
    def evaluate_explanation(self, explain_function, model, test_dataset, explain_name):
        ...
        for data in test_dataset:
            edge_to_id = {}
            for idx, edge in enumerate(zip(*[data.edges()[0].cpu().numpy(),data.edges()[1].cpu().numpy()])):#enumerate((zip(*data.edges().cpu().numpy()))):
                edge_to_id[edge] = idx
            g = data.to_networkx()
            ...
            for node_idx in pbar:
                ...
                for u in list(g.neighbors(node_idx)):
                    ...
                    if data.ndata['x'][u][1].item():
                        blue_ids.append(eid)
                    elif data.ndata['x'][u][2].item():
                        red_ids.append(eid)
                    else:
...
if __name__ == '__main__':
    A = Saturation(10, 1, True, 'GraphConvWL')
    A.run()
