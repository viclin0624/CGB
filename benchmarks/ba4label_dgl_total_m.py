...
class BA4labelDataset(DGLDataset):
    ...
    def __init__(self, graphs_num = 1000, nodes_num = 25, m = 1, perturb_dic = {}):
    ...
    def process(self):
        ...
        for _ in range(self.graphs_num):
            ...
            if which_type == 0:
                if perturb_type == 0:
                    G, role_id, plug_id = build_graph(self.nodes_num, self.basis_type, [], start = 0, m = m)
                else:
                ...
                G, role_id, plug_id = build_graph(self.nodes_num-10-perturb_type, self.basis_type, list_shapes, start = 0, m = m)
...
def run(exp_name: str = typer.Argument(..., help="Experiment to run"), 
        readout: str = typer.Argument(..., help="Mean or Max"),
        layer: int = typer.Argument(None, help="layer Exp")):
    NODES_NUM = 25
    BATCH_SIZE = 16
    EPOCHS_NUM = 300
    EXPS_NUM = 10
    GRAPH_NUM = 3000
    M_LIST = [1,5]
    device = device = torch.device('cuda:'+str(int(exp_name)%4))
    if layer == None:
        LAYERS_LIST = [1,2,3,4,5,6]
    else:
        LAYERS_LIST = [layer]
        device = device = torch.device('cuda:'+str(int(layer)%4))
    if int(exp_name) >= 4:
        NODES_NUM = 50
    
    exp_to_perturb = {1:{}, 2:{3:'three_cycle', 4:'four_cycle', 6:'six_cycle'}, 3:{4:'square_diagonal'}, 4:{}, 5:{3:'three_cycle', 4:'four_cycle', 6:'six_cycle'},
    6:{4:'square_diagonal'}, 7:{7:'multi_motif'}}
    perturb_dic = exp_to_perturb[int(exp_name)]
    result_dic = {}
    m = None
    for layers in tq(LAYERS_LIST):
        data = BA4labelDataset(graphs_num = GRAPH_NUM, m = None,nodes_num=NODES_NUM, perturb_dic = perturb_dic)
        ...
        for _ in tq(range(EXPS_NUM)):
            model = Net2(data.num_node_features, data.num_classes, layers, True,
                                'GraphConvWL', readout = readout).to(device)
    ...
    if m == None:
        ...
        record_exp(result_dic, int(exp_name), mlist = M_LIST, layerlist = LAYERS_LIST)
