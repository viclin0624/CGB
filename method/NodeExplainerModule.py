...
class NodeExplainerModule(nn.Module):
    ...
    def forward(self, graph, n_feats):
        ...
        #new_n_feats = n_feats * self.node_feat_mask.to(self.device).sigmoid()
        edge_mask = self.edge_mask.to(self.device).sigmoid()

        # Step 2: Add compute logits after mask node features and edges
        new_logits = self.model(graph, n_feats, edge_mask)
